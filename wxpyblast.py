	...
	try:
		...
		for blast_record in blast_records:
			if(header_written == False):
				print >> reportfile_handle, "File written by wxpyblast version", __version__, "on", datetime.datetime.today().strftime("%Y/%m/%d %I:%M:%S %p")
				print >> reportfile_handle, "Input file", input_filename
				print >> reportfile_handle, "Results generated by", blast_record.application, "version", blast_record.version,\
				"using", blast_record.database, "database"
				print >> reportfile_handle, "Parsing tweakables, max_e =", tweakables["max_e"],\
				"max_results =", tweakables["max_results"], "min_ident =", tweakables["min_ident"], "min_coverage =",\
				tweakables["min_coverage"], "regex-filter =", tweakables["filter_title_regex"]
...
class blastThread(threading.Thread):
	...
	Note: In order to not flood NCBI servers and possibly end up on a block list, we have every thread grab a 5s lock before sending the request
	...
	def __init__(self,sequence_record,record_id,database,program,megablastFlag):
		threading.Thread.__init__(self)
		self.sequence_record = sequence_record
		self.record_id = record_id
		self.database = database
		self.program = program
		self.megablastFlag = megablastFlag
	def run(self):
			...
			while blast_status is 0:
				try:
					with blastThread.ncbi_wait:
						time.sleep(5)
					result_handle = NCBIWWW.qblast(self.program, self.database, self.sequence_record, megablast = self.megablastFlag)					
...
class MainWindow(wx.Frame):
	def __init__(self, parent, id, title):
		...
		min_queryHeaderText = wx.StaticText(panel, -1, "Query Options")
		...
		mainBox.Add(min_queryHeaderText,0,wx.ALIGN_CENTER_HORIZONTAL)
		mainBox.Add(database_selectorText,0,wx.EXPAND)
		mainBox.Add(self.database_selector,0,wx.EXPAND)
		mainBox.Add(program_selectorText,0,wx.EXPAND)
		mainBox.Add(self.program_selector,0,wx.EXPAND)
		mainBox.Add(seperatorLine,0,wx.EXPAND)
	...
	def parseFailed(self):
		dlg = wx.MessageDialog(self, "No results returned? Check program/database.", "Parsing XML Failed", wx.OK|wx.ICON_ERROR)
	...
	def OnOpenFasta(self,e):
		...
		if dlg.ShowModal() == wx.ID_OK:
			...
			dlg.Destroy()
			xmlDialog = wx.FileDialog(self, "Save XML as", self.fasta_dirname, input_filename+".xml", "XML Blast Results (*.xml)|*.xml"
			...
			if xmlDialog.ShowModal() == wx.ID_OK:
				...
				xmlDialog.Destroy()
				reportDialog = wx.FileDialog(self, "Save report as", self.xml_dirname, input_filename+"_report.txt", "Report (*.txt)|*.txt"
					, wx.SAVE | wx.OVERWRITE_PROMPT)
				if reportDialog.ShowModal() == wx.ID_OK:
					self.report_filename=reportDialog.GetFilename()
					self.report_dirname=reportDialog.GetDirectory()
					...
					if self.database_selector.GetValue() == "megablast":
						program = "blastn"
						megablastFlag = True
					else:
						program = self.program_selector.GetValue()
						megablastFlag = False
					for seq_record in records :
						blastEngine = blastThread(seq_record.format("fasta"), seq_record.id,self.database_selector.GetValue(),
							program, megablastFlag).start()
